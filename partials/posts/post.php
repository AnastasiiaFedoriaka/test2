...
if($result = $conn->query($sql)):
	foreach ($result as $row):
		...
		$category = $categoryResult->fetch_assoc(); //Отримання даних в масиві

		/*
		1.Зробити посилання на лайк(окрема сторінка, на яку ми будем поосилатись за допомогою аякс)
		    ...
		    <div class="meta-wrap align-items-center">
		      	...
		      	<p class="meta">
		      		<?php
		      			...
		      			$likesResualt = $conn->query($likesSQL);
		      			//$likeUserResul = null;
		      			
		      			if(isset($_COOKIE['user'])) {
		      				$lireUserSQL = 'SELECT count(*) as total FROM user_post_likes WHERE post_id=' . $row['id'] . 'user_id=' . $_COOKIE['user'];
		      		...
		      		?>
		      		" data-id="<?php echo $row['id']; ?>"><i class="icon-heart"></i><b><?php echo $likesResualt->fetch_assoc()['total']; ?></b></span>
